...
- **Verbatim vendored data fixtures** — third-party schemas and payloads checked in unmodified as test input, of which `tests/fixtures/hindsight-openapi.json` (13,422 lines) is the only current instance. Same reasoning as `records/`: it is not authored here, it cannot be split without ceasing to be the thing it vendors, and baselining it would park an unmovable 13k-line entry at the top of the file that is supposed to measure migration progress. The exemption is by nature, not by path, so a plan adding such a fixture states why it qualifies.
