...
- [ ] All **seven** broken pins are amended, none deleted: `test_review_wiring.py:51` (Task 3), `test_adversary_workflow.py:196` (Task 3), `test_adversary_workflow.py:161-172` (Task 3), `test_adversary_workflow.py:106-112` (Task 4), `test_adversary_workflow.py:88` `test_adversary_helper_exists_and_is_config_gated` (Task 4), `test_deep_review_wiring.py:28` `test_deep_review_gated_on_config_flag` (Task 4), `test_review_wiring.py:75` (Task 6). The spec §4.1 four-pin table is short by **three** pins: the `:161-172` fifth pin the plan itself caught, plus the two Task 4 pins that fired SG-1 and were amended under the orchestrator's clearance of 2026-09-10 (retargeted at the live single home of each rule — review/step.py's `run_adversary` pre-check and `run_deep_review` pre-check — never relaxed, never deleted).
