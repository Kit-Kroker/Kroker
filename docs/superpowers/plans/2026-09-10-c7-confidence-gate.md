...
- **Merge never auto-approves under this design** (ruling OQ2), and this must fall out of the *data*, not a special case in the code. Merge-gate decisions are simply not sampled — no ledger row is written (`CalibrationSample.outcome_label` is NOT NULL, so an unlabelled row cannot exist), so `verdict_for` returns `insufficient_data` forever. Do not write `if gate == "merge"` anywhere.
