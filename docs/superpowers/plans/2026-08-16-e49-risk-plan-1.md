...
def qa_factors(
    ...
    if not coverage_collected:
        ...
        rates = [
            r.covered.value for r in cap.coverage if r.covered.state is CollectionState.MEASURED
        ]
        gap = (
...
def _drivers(factors: tuple[Factor, ...]) -> tuple[Driver, ...]:
    ...
    """
    rows = [
        Driver(factor_key=f.key, value=f.value.value, contribution=f.value.value * f.weight)
        for f in factors
        if f.collected
    ]
...
def compose(factors: tuple[Factor, ...], *, label: str) -> Composite:
    ...
    if missing:
        ...
        value = Measurement.measured(sum(f.value.value * f.weight for f in factors) / total)
...
def _cmap(*caps) -> CapabilityMap:
    return CapabilityMap(capabilities=tuple(caps), collected=Measurement.measured(1.0))
...
def build(cmap: CapabilityMap, *, collected_categories: frozenset[str]) -> UnifiedRiskMap:
    ...
    sensitivity_collected = any(c.sensitivity for c in cmap.capabilities) or False
    ...
    for cap in sorted(cmap.capabilities, key=lambda c: c.bc_id):
        ...
        qa = compose(
            qa_factors(cap, coverage_collected=bool(cap.coverage), testability_collected=True),
            label="qa",
        )
        rows.append(
            CapabilityRisk(
    ...
    return UnifiedRiskMap(
        capabilities=tuple(rows), system=SystemRisk(), collected=Measurement.measured(1.0)
    )
```
...
```python
def risk_key(project: str, tree_hash: str, map_digest: str, rules_sha: str) -> str:
...
def load(*, project: str, tree_hash: str, map_digest: str, rules_sha: str) -> UnifiedRiskMap | None:
...
def store(
    *, project: str, tree_hash: str, map_digest: str, rules_sha: str, out: UnifiedRiskMap
) -> bool:
    ...
    if out.collected.state is not CollectionState.MEASURED:
        return False
    cache.put(cache.risk_key(project, tree_hash, map_digest, rules_sha), out.model_dump_json())
    return True
```
...
def _inp(cmap: CapabilityMap) -> AssessRiskInput:
    return AssessRiskInput(
        project="p", tree_hash="t", capability_map=cmap, collected_categories=ALL
    )


@pytest.mark.asyncio
async def test_it_scores_a_capability_map():
    out = await assess_risk(
        _inp(CapabilityMap(capabilities=(capability(),), collected=Measurement.measured(1.0)))
    )
...
async def test_a_second_call_hits_the_memo():
    inp = _inp(CapabilityMap(capabilities=(capability(),), collected=Measurement.measured(1.0)))
...
async def assess_risk(inp: AssessRiskInput) -> UnifiedRiskMap:
    ...
    sha = risk_rules_sha()
    hit = risk_memo.load(
        project=inp.project, tree_hash=inp.tree_hash, map_digest=digest, rules_sha=sha
    )
    if hit is not None:
        return hit
    try:
    ...
    risk_memo.store(
        project=inp.project, tree_hash=inp.tree_hash, map_digest=digest, rules_sha=sha, out=out
    )
    return out
```
...
def test_an_uncollected_assess_phase_must_not_carry_a_risk_map():
    ...
    with pytest.raises(ValidationError, match="did not assess"):
        _assessment_with(
            PhaseId.ASSESS,
            Measurement.not_collected("stub"),
...
def test_phase_owner_no_longer_owes_assess():
    ...
    assert set(PHASE_OWNER) == {PhaseId.REPORT, PhaseId.GENERATE, PhaseId.FINISH}
...
async def _assess(
    ...
    if discover_out.map is None:
        return no_assess(
            "discover produced no capability map, so there is no capability set to assess"
        )
    collected = sorted(
        category
        for signal, categories in CATEGORIES.items()
        for category in categories
        if scan_out.result_for(signal) is not None and scan_out.measured(signal, category)
    )
    ...
    if risk.collected.state is not CollectionState.MEASURED:
        return AssessOutcome(result=PhaseResult(phase=PhaseId.ASSESS, collected=risk.collected))
    return AssessOutcome(
        result=PhaseResult(phase=PhaseId.ASSESS, collected=Measurement.measured(1.0)), risk=risk
    )
```
