...
| B4 | **Fleet back-pressure (STOP_IF)** — a cap on decisions pending human review; new runs refuse to start while the queue is over the cap | both | ✅ Fixed | shipped — `pending_run_count` (counts RUNS not items; fail-closed on unqueryable OPEN runs), `check_fleet_capacity` (>= boundary, so a configured N means N), `fleet_pending_cap`, and `guard_fleet_capacity` in `src/sdlc/dashboard/fleet.py`, enforced at all three client-side FeatureWorkflow start paths: `cli.py`'s `start` (refusal → operator-readable message, exit 1), `operator/tools.py`'s `start_run` (→ ToolError telling the model not to retry), `dashboard/api.py`'s `POST /runs` (→ 429, never 502). Opt-in via `SDLC_FLEET_PENDING_CAP` — unset means no cap, and the fleet fetch is skipped entirely. Accepted gap, named not fixed: the count is FeatureWorkflow-only, so CrewTaskWorkflow children stalled on a human gate are invisible to the cap. See docs/superpowers/specs/2026-09-09-b4-fleet-backpressure-design.md and the implementation plan for the full design |
...
| C3 | **Fail-closed on a missing gate** — an absent/unconfigured required check yields `MISCONFIGURED` and a failing verdict, never a quiet green run | factory | ✅ **Fixed** | `gate.py`: `MERGE_REQUIRED_CHECKS` (the manifest, beside `ABSOLUTE_FLOOR` at `:71`) is the set the merge gate must see; `evaluate_quality_gate` synthesizes a failing `MISCONFIGURED` check through `build_check` (`:99`) for every absent name, at that name's manifest classification, and echoes it in `GateReport.checks`. Absolute absence is terminal, advisory absence is waivable at the audited human gate. No new verdict enum. The same pass re-asserts `ABSOLUTE_FLOOR` on input, closing the sibling hole where a directly-constructed `CheckResult` demotes a floor check and waives it. Contract: `merge.md` MERGE-1.6. |
...
| C7 | **Nothing stands behind self-reported confidence** — under SOFT policy a proposer's own `confidence >= threshold` skips the human gate outright (`role_host.py:55-75`, consumed `:224`; merge's copy at `merge/step.py:84-100`, consulted `:399`) | *found in audit* | ✅ **Fixed** | shipped — the gate calibration ledger: a SOFT auto-approve now requires `confidence >= threshold` **and** a `calibrated` verdict for the `(gate, author_model)` bucket, the verdict a REQUIRED no-default parameter on the single rule (`auto_decision_for`) so "no evidence was fetched" can never read as "the evidence passed"; `insufficient_data` (cold start, unknown model, under-sampled), `uncalibrated`, and a failed lookup all resolve the same way — no auto-decision, the human waits. Strictly one-directional: an auto-approve that would have fired may now not fire, nothing that waited can start skipping, and HARD/OFF/None-confidence paths never even perform the ledger read. Labels computed at retro from realized-outcome proxies — continuous plan-drift ratio for `plan`, fix-attempt rate capped at `max_fix_attempts` for `architecture`, both aggregated over the run's TASK rows (`stage=="code"`) so zero-attempt stage rows cannot dilute the label; benchmark mode prefers the per-stage `llm_judge` score, and contract 1.0/0.0 pass-fail values never displace the proxy. Storage: append-only SQLite `gate_calibration` table on the board substrate (ADR-19: adapters, not substrate), reached only through activities; N=20 sample floor, rolling 200-sample window (superseded rows stay on disk for audit), and benchmark/production-proxy samples never pooled — `source` is folded into `bucket_key`. Merge's SOFT auto-approve is off because its bucket is never sampled: no attributable post-merge signal exists (`outcome_label` is NOT NULL, so an unscorable row cannot be written) — the behaviour change was accepted at the user gate and the post-merge signal filed as its own register-row proposal. Agreement constants inherit 0.75/0.15 from the rubric-judge loop as a starting position, to be re-derived from real samples. The two logically identical `_auto_decision_for` copies collapsed into one home at `src/sdlc/calibration/decision.py`. Contracts: MERGE-1.9, RETRO-1.6, `src/sdlc/calibration/AGENTS.md`. All nine open questions ruled as recommended at the 2026-09-10 user gate (spec Rulings section). See docs/superpowers/specs/2026-09-10-c7-confidence-gate-design.md and the implementation plan |
| C8 | **A lens that did not run is indistinguishable from a lens that approved** — the fail-open legs of the review stage leave no tombstone | *found in audit* | ✅ **Fixed** | shipped — `LensOutcome` (validator makes an absent-and-approved outcome unconstructible; every absent state carries a reason) over the four-state `LensPresence` enum `DECLARED_ABSENT` / `UNDECLARED_ABSENT` / `NOT_REACHED` / `PRESENT`, produced by the single pure classifier `classify_lens` in `src/sdlc/stages/review/lenses.py` (no `ctx`, no I/O; derived from the same facts the runner's own pre-check consults, so a tombstone cannot disagree with the predicate that gated the run). `primary_admits` / `backstop_admits` transcribe the two shipped admission rules exactly — including the primary's fail-on-any-rejection vs the backstop's fail-only-on-blocking asymmetry — so C8 changes what absence SAYS, never what the task layer DOES (ruling OQ1: no presence state blocks delivery). `TaskResult.lens_outcomes` is populated on both code-step return sites. At the merge gate, `review_lenses_present` joins `MERGE_REQUIRED_CHECKS` as required-ADVISORY — fails on `UNDECLARED_ABSENT` or a missing outcome (the empty-list defense against producers predating the field), `DECLARED_ABSENT`/`NOT_REACHED` pass but are named in the detail; waivable only through the audited `GateOverride` — and `review_severity` now grades only PRESENT reviewer lenses through the same `primary_admits`, so task and merge layers cannot diverge on what "approved" means. The `:802` compound guard (`if review is not None`) is deleted: "primary off, adversary on" used to run no lens at all — a recorded position reversed, with the argument carried by its amended test. Contracts: REVIEW-1.6, CODE-1.6, MERGE-1.8. Residuals and rulings: benchmark-corpus recording of lens outcomes split out at the user gate as its own future row (proposal filed at `.workspace/tasks/2026-09-10-benchmark-lens-outcome-records.md`); the dead TaskHost twins deleted (OQ4) so the absence predicate is single-homed in the runner; the plan's Task 5/6 sequencing bent by orchestrator ruling 2026-09-10 — the manifest entry rode with the helper because the AST census in `tests/test_required_checks_manifest.py` makes helper-without-entry permanently red — with rationale recorded in the commit; spec §4.1's pin inventory was short by three pins, all amended by retargeting at the live source, never relaxed. See docs/superpowers/specs/2026-09-09-c8-review-lens-tombstone-design.md and the implementation plan |
