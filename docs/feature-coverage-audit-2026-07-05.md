...
| FR | Status | Note |
|---|---|---|
| FR-301 (hard/soft/off, confidence threshold, approve/reject/revise, `MAX_GATE_ROUNDS`) | ⚠️ Partial — **soft gates don't actually auto-approve** | `GatePolicy`, `GateOutcome`, `_revisable_stage`, `max_gate_rounds` are all correctly implemented (`feature.py:259-313`, `models.py:305-307`). But `_gate()`'s SOFT branch (`feature.py:270`) only fires `if policy == SOFT and auto_decision and auto_decision.approved` — and **no call site in `feature.py` ever passes `auto_decision`**. `PipelineConfig.gates["plan"] = SOFT` by default (`models.py:289`), yet in practice every gate, soft or hard, waits for a human signal today. The confidence-threshold mechanism described in the PRD (soft = "auto-approve iff `proposal.confidence >= threshold`") has no threshold field anywhere and no code path that reads a proposer's confidence to decide. This is a real, currently-silent gap: soft-gate config exists but has no effect. |
...
| SC-6 (soft-gate override rate <5%) | — Not measurable, and structurally can't be yet | Since soft gates never actually auto-approve (FR-301 gap above), there's no soft-gate decision population to sample. |
...
| **P3** Hindsight memory + confidence-gated soft gates | SC-4/SC-6 measurable | Memory mechanism (recall/retain/watermark) is ready; confidence-gated soft gates are not wired (FR-301 gap blocks this phase's core deliverable). |
...
1. **Wire soft-gate auto-approval or remove `SOFT` from the default config.** Right now `PipelineConfig.gates["plan"] = SOFT` (`models.py:289`) implies auto-approval that structurally cannot happen — `_gate()` never receives an `auto_decision`. This is silent: a project operator reading `pipeline.yaml`-equivalent config would reasonably expect the plan gate to sometimes auto-proceed, and it never will. Either thread a confidence-scored `auto_decision` from the plan/architecture proposer into `_gate()`, or default `gates["plan"]` to `HARD` until that exists, so config doesn't overclaim.
