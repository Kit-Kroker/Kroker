...
| C2 | **Test freeze during fix loops** — the harness session repairing code may not edit or weaken the contract's tests; protected paths per task | both | ✅ Gap verified | `harness/containment.py` already carries the vocabulary (`PATH_MATCHES` at `:44`, `DENY`/`ESCALATE` at `:52`); the work is overlaying per-task paths onto a policy that is a static, hashed asset |
