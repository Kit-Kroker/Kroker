...
- [ ] ⚠️ **FR-911** `AssessmentWorkflow` EDCR DAG, report-after-assess, no
  ...
  — discover is now built and runs measured end-to-end. **2026-08-16 (E-49 plan 1):**
  the stub count dropped from four to three and `PHASE_OWNER` lost its `ASSESS` entry
  — assess is now built and the phase is measured with no model in the loop.
