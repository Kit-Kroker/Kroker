...
- [x] **FR-916** STRIDE + vuln classification + control coverage + composites with 1–3 specific drivers (E-49). **Both plans landed 2026-08-16.** Plan 1: deterministic baseline composites, severity, factors, rules_sha, memo caching, and `assess_risk` activity seam. Plan 2: lift shared quote verifier to `assessment/verification.py` (RD6), proposer models and contracts, `render_risk_prompt`, `apply_judgment` with layer-scoped degradation (RD7), `risk` role, memo seam and cache key extension (P2-D3), `verify_risk_refs` activity, and assessment workflow wiring.
...
- [x] **E-49 — `UnifiedRiskMap` + risk proposers** → FR-916. Conforms to the
  ...
  **Plan 2 landed 2026-08-16:** the judgment layer — lifted the shared quote verifier to `assessment/verification.py` (RD6), defined proposer models and contracts with `UnifiedRiskMap.judgment` tracking, implemented `render_risk_prompt` and `apply_judgment` with layer-scoped degradation (RD7), registered the `risk` role, extended memo caching with store refusal under transient failures (P2-D3), wired `verify_risk_refs` activity, and integrated full judgment flow into `AssessmentWorkflow._assess` with Temporal e2e verification.
