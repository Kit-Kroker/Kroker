...
from sdlc.assessment.risk.models import (
    ...
    CriticalityRating, ProposedControl, ProposedThreat, ProposedVulnerability,
    RiskProposal, RiskSource, RiskVerification, StrideCategory,
    ThreatAssessment, UnifiedRiskMap, Severity, Vulnerability,
    VulnerabilityClass,
)
...
def _threats(source=RiskSource.BASELINE):
    return tuple(ThreatAssessment(category=c, applicable=False,
                                  rationale="baseline", source=source)
                 for c in StrideCategory)


def _controls():
    return tuple(ControlCoverage(family=f, state=ControlState.PRESENT,
...
@pytest.mark.parametrize("model", [ProposedThreat, ProposedVulnerability,
                                   ProposedControl])
...
def test_no_proposer_row_can_author_a_number(model):
    ...
    forbidden = {"severity", "criticality", "security", "qa", "unified",
                 "factors", "drivers", "value", "weight"}
...
def test_row_ids_are_distinct_across_the_three_families():
    ...
    cannot collide."""
    t = ProposedThreat(bc_id="BC-001",
                       category=StrideCategory.SPOOFING,
                       applicable=True, rationale="r")
    v = ProposedVulnerability(key="BC-001",
                              classification=VulnerabilityClass.CONFIRMED,
                              stride_category=StrideCategory.SPOOFING,
                              rationale="r")
    c = ProposedControl(bc_id="BC-001",
...
def test_rows_returns_every_family_in_declaration_order():
    p = RiskProposal(
        threats=[ProposedThreat(bc_id="BC-001",
                                category=StrideCategory.SPOOFING,
                                applicable=True, rationale="r")],
        controls=[ProposedControl(bc_id="BC-001",
                                  family=ControlFamily.VALIDATION,
                                  state=ControlState.ABSENT, rationale="r")])
    assert [r.row_id for r in p.rows] == [
        "threat:BC-001:spoofing", "control:BC-001:validation"]
