...
from sdlc.assessment.discover.apply import (
    ApplyResult, LockedCandidate, StampedProposal, build_map,
)
from sdlc.assessment.discover.map import (
    CITATION_GUARD_MAX_UNRESOLVED, CandidateDisposition, DiscoverAction,
    DiscoverProposal, DispositionSource, guard_tripped,
)
...
def _verification(total: int, unresolved: int) -> RefVerification:
    return RefVerification(proposal=DiscoverProposal(),
...
def test_total_references_reaches_the_map():
    ...
    applied = ApplyResult(locked=(locked,),
                          stamped=StampedProposal(dispositions=(disp,)))
    bc_of = {"C-01": "BC-001"}
