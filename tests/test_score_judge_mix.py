...
from sdlc.benchmarks.models import (BenchmarkOutcome, BenchmarkRecord,
...
def _rec(case, judge, score=0.8):
    return BenchmarkRecord(
        run_id="r1", bench_run_id="b1", case_id=case,
        scope=BenchmarkScope.STAGE, stage="clarify", role="clarify",
        model="m", quality=QualityScore(score=score, judge=judge),
        speed=SpeedBag(wall_clock_s=1.0, started_at=_NOW, ended_at=_NOW),
        outcome=BenchmarkOutcome.PASS)


def test_single_judge_kind_produces_no_note():
    assert judge_mix_notes([_rec("c1", "llm_judge"),
                            _rec("c1", "llm_judge")]) == []


def test_mixed_judge_kinds_in_one_case_produce_a_note():
    notes = judge_mix_notes([_rec("c1", "llm_judge"),
...
def test_non_scoring_judges_do_not_count_as_a_mix():
    ...
    assert judge_mix_notes([_rec("c1", "staged_rubric"),
                            _rec("c1", "oracle"),
                            _rec("c1", "deep_review"),
                            _rec("c1", "contract")]) == []
