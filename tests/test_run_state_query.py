...
from sdlc.models import (GateDecision, GateOutcome, IdeaBrief, PipelineConfig,
                         ProjectMode, RoleUsage)
...
def _wf(**overrides):
    ...
    wf = FeatureWorkflow()
    wf._idea = overrides.pop("idea", IdeaBrief(
        title="Add SSO", description="d", mode=ProjectMode.BROWNFIELD,
...
def test_current_stage_is_the_last_stage_started():
    wf = _wf(_trace=[
        RunEvent(seq=1, at=AT, kind=RunEventKind.STAGE_STARTED,
                 stage="clarify"),
        RunEvent(seq=2, at=AT, kind=RunEventKind.STAGE_ENDED, stage="clarify"),
        RunEvent(seq=3, at=AT, kind=RunEventKind.STAGE_STARTED,
                 stage="architecture"),
    ])
