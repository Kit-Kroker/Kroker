...
def test_workflow_id_is_per_run_not_per_repository():
    ...
    assert assess_workflow_id("/x/my-repo", now) == \
        "assess-my-repo-20260810T101500Z"
...
def test_two_ids_for_one_repo_differ():
    a = assess_workflow_id("/x/r", datetime(2026, 8, 10, 10, 15, 0,
                                            tzinfo=timezone.utc))
    b = assess_workflow_id("/x/r", datetime(2026, 8, 10, 10, 16, 0,
                                            tzinfo=timezone.utc))
    assert a != b
