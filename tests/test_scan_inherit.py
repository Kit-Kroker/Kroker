...
from sdlc.assessment.scan.models import (
    ...
    C_FRAMEWORK_DEFAULTS, C_TESTS_PRESENT, ScanSignalId,
)
...
def _finding(signal: str, rule: str, path: str = "x.py",
             key: str = "") -> TriageFinding:
    return TriageFinding(signal=signal, rule=rule, severity="high",
                         detail="d", path=path, fix_class=FixClass.MECHANICAL,
                         key=key)
...
def _triage(*signals: SignalResult, tests: float = 2.0) -> RepoTriage:
    ...
    return RepoTriage(
        ...
        readiness=Readiness(buildable=ok, runnable=ok,
                            tests_present=Measurement.measured(tests),
                            structure_discernible=ok, verdict=Verdict.READY),
        signals=list(signals))


def _sig(signal: str, version: int, findings: list[TriageFinding],
         metrics: dict[str, Measurement] | None = None) -> SignalResult:
    return SignalResult(
...
def test_five_signals_have_an_inherited_half():
    ...
    assert set(halves) == {ScanSignalId.SS1, ScanSignalId.SS2,
                           ScanSignalId.SS3, ScanSignalId.QS1,
                           ScanSignalId.QS4}
...
def test_ss1_credential_storage_is_measured_with_the_finding_count():
    f = _finding("secrets", "aws_key", ".env", "abc")
    cats = inherited_halves(
        _triage(_sig("secrets", 2, [f])))[ScanSignalId.SS1].categories
...
def test_a_not_collected_triage_signal_propagates_its_reason():
    sig = SignalResult(signal="secrets", version=2,
...
def test_only_inherited_categories_appear_in_the_half():
    ...
    cats = inherited_halves(_triage(_sig("secrets", 2, [])))[
        ScanSignalId.SS1].categories
...
def test_ss3_inherits_framework_defaults_from_misconfig():
    ...
    cats = inherited_halves(_triage(_sig("misconfig", 2, [f])))[
        ScanSignalId.SS3].categories
...
def test_ss2_inherits_direct_dependencies_and_is_the_whole_signal():
    ...
    half = inherited_halves(_triage(_sig("dependencies", 1, [f])))[
        ScanSignalId.SS2]
...
def test_qs1_inherits_the_test_count_from_baselines_metric_not_its_findings():
    ...
    sig = _sig("baseline", 2, [],
               {"tests_present": Measurement.measured(7.0)})
...
def test_qs4_ci_present_is_the_absence_of_baselines_no_ci_finding():
    ...
    with_ci = inherited_halves(_triage(_sig("baseline", 2, [])))[
        ScanSignalId.QS4].categories
    assert with_ci[C_CI_PRESENT].value == 1.0

    without = inherited_halves(_triage(_sig("baseline", 2,
                                            [_finding("baseline", "no_ci")])))[
        ScanSignalId.QS4].categories
    assert without[C_CI_PRESENT].value == 0.0


def test_qs4_ci_present_is_not_collected_when_baseline_did_not_run():
    sig = SignalResult(signal="baseline", version=2,
                       collected=Measurement.not_collected("boom"))
