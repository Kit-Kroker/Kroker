...
UNCONFIRMED = StageGatePending(key="unconfirmed#1", gate="unconfirmed",
...
def snap():
    return FleetSnapshot(
        ...
        runs=[RunState(run_id="feature-add-sso", title="Add SSO",
                       mode="brownfield", status="awaiting:architecture",
                       started_at=AT)])
...
def test_decide_leaves_decided_by_as_human(client, submitted):
    client.post("/runs/feature-add-sso/decide",
                json={"key": "architecture#1", "outcome": "approve"},
                headers={"X-Actor": "human:mika"})
...
def test_decide_404s_when_the_key_is_not_pending(client):
    r = client.post("/runs/feature-add-sso/decide",
                    json={"key": "merge#9", "outcome": "approve"})
...
def test_answer_routes_a_clarify_key_to_the_answer_question_signal(
        client, submitted):
    r = client.post("/runs/feature-add-sso/answer",
                    json={"key": "Q1", "text": "OIDC"})
    assert r.status_code == 200
...
def test_an_unconfirmed_submit_is_reported_as_200_not_an_error(client):
    ...
    r = client.post("/runs/feature-add-sso/decide",
                    json={"key": "unconfirmed#1", "outcome": "approve"})
    assert r.status_code == 200
...
def test_start_run_builds_the_workflow_id_from_the_title(start_client):
    r = start_client.post("/runs", json={
        "title": "Add SSO to portal", "description": "d",
        "mode": "brownfield", "repo": "git@example:acme/portal"})
    assert r.status_code == 200
...
def test_start_run_409s_on_a_duplicate_id(start_client):
    r = start_client.post("/runs", json={
        "title": "taken", "description": "d", "mode": "greenfield"})
