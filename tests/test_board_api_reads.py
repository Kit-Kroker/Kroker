...
def client(tmp_path):
    ...
    _, vid = seed.publish_artifact_version(
        "proj", "architecture", "run-1", b'{"overview":"first"}',
    ...
    seed.set_task_authoritative("proj", plan_v, "T01",
...
def test_project_detail_lists_artifacts_and_task_rollup(client):
    ...
    assert body["stats"]["tasks_by_status"] == {"in_progress": 1,
                                                "pending": 1}
...
def test_version_content_is_returned(client):
    r = client.get(f"/projects/proj/artifacts/architecture/versions/"
                   f"{client.arch_v}")
    assert r.status_code == 200
    body = r.json()
...
def test_events_since_filters(client):
    ...
    tail = client.get(
        f"/projects/proj/events?since={everything[0]['id']}").json()
...
def test_unknown_artifact_is_404(client):
    assert client.get(
        "/projects/proj/artifacts/requirements").status_code == 404
...
def test_missing_blob_is_410(client):
    ...
    r = client.get(
        f"/projects/proj/artifacts/architecture/versions/{v['id']}")
...
def test_content_over_cap_is_truncated(tmp_path):
    ...
    _, vid = seed.publish_artifact_version(
        "p", "plan", "r", b"x" * (api_mod.MAX_CONTENT_BYTES + 10),
        actor="workflow:r")
    seed.close()
