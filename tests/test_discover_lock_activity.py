...
from sdlc.capability.models import (
    AttachMethod, CapabilityFingerprint, ProposedCapability, SignalTier,
)
...
def _proposed(local_key="C-01", *routes) -> ProposedCapability:
    return ProposedCapability(local_key=local_key,
                              fingerprint=_fp(*(routes or ("POST /pay",))))


async def test_the_lock_mints_ids_and_persists_them():
    out = await discover_lock(DiscoverLockInput(
        project="acme", run_id="run-1",
        proposed=[_proposed("C-01"), _proposed("C-02", "GET /orders")]))
    assert {a.local_key for a in out.attachments} == {"C-01", "C-02"}
    assert all(a.method is AttachMethod.FIRST_DISCOVERY
...
async def test_a_second_lock_on_the_same_fingerprints_reattaches():
    ...
    first = await discover_lock(DiscoverLockInput(
        project="acme", run_id="run-1", proposed=[_proposed("C-01")]))
    second = await discover_lock(DiscoverLockInput(
        project="acme", run_id="run-2", proposed=[_proposed("C-01")]))
...
async def test_an_empty_proposal_does_not_move_the_registry_version():
    ...
    out = await discover_lock(DiscoverLockInput(
        project="acme", run_id="run-1", proposed=[]))
...
async def test_an_unreachable_store_raises_rather_than_degrading(tmp_path, monkeypatch):
    ...
    with pytest.raises(Exception):
        await discover_lock(DiscoverLockInput(
            project="acme", run_id="run-1", proposed=[_proposed("C-01")]))
