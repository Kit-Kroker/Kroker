...
def _summary(*events):
    return build_run_summary(run_id="r", mode="brownfield", outcome="deployed",
                             trace=list(events), memory_enabled=False,
...
def test_an_empty_dimension_string_reads_as_no_dimension():
    ...
    s = _summary(
        _ev(0, RunEventKind.CLARIFICATION_ASKED, question_id="Q1",
            question="q?", dimension=""),
        _ev(1, RunEventKind.CLARIFICATION_ANSWERED, question_id="Q1",
            answered_by="suggested"))
    assert s.clarifications[0].dimension is None
