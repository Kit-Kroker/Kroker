...
async def test_tool_escalations_do_not_burn_question_budget():
    ...
    settings = GateSettings(
        default_gate_policy=GatePolicy.OFF,
        gates={
            "tool_approval": GateConfig(policy=GatePolicy.OFF),
    ...
    async with await WorkflowEnvironment.start_time_skipping(
        ...
        async with Worker(
            ...
            res = await env.client.execute_workflow(
                CrewTaskWorkflow.run,
                _inp(gate_settings=settings, max_tool_escalations=5, rounds_max=2),
                id=f"crew-{uuid.uuid4()}",
                task_queue=TASK_QUEUE,
            )
    assert res.run.exit_code == 0
...
async def test_a_crews_pending_item_names_its_parent_run():
    ...
    async with await WorkflowEnvironment.start_time_skipping(
        ...
        async with Worker(
            ...
            for _ in range(100):
                try:
                    pending = await child_handle.query(CrewTaskWorkflow.pending_decisions)
                    if pending:
                        break
                except Exception:
                    pass
