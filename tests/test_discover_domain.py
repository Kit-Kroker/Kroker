...
from sdlc.assessment.discover.models import (
    EntityOwnership, OwnershipOutcome, OwnershipReport,
)
...
def test_an_owned_entity_carries_its_owner_and_verb():
    ...
    rep = _report(EntityOwnership(
        entity="orders", outcome=OwnershipOutcome.OWNED, owner="BC-001",
        verb=OwnershipVerb.OWNS, rule="declaration", claimants=("BC-001",)))
    out = consolidate(rep, [])
    row = out.entities[0]
...
def test_the_three_unowned_outcomes_stay_distinct():
    ...
    re-collapse them."""
    rep = _report(
        EntityOwnership(entity="a", outcome=OwnershipOutcome.CONFLICT,
                        rule="tie", claimants=("BC-001", "BC-002")),
        EntityOwnership(entity="b", outcome=OwnershipOutcome.UNDIRECTED,
                        rule="reads", claimants=("BC-001",)),
        EntityOwnership(entity="c", outcome=OwnershipOutcome.UNCLAIMED,
                        rule="none"),
    )
    out = consolidate(rep, [])
    got = {e.entity: e.outcome for e in out.entities}
    assert got == {"a": OwnershipOutcome.CONFLICT,
...
def test_readers_are_carried_from_claimants_minus_the_owner():
    ...
    rep = _report(EntityOwnership(
        entity="orders", outcome=OwnershipOutcome.OWNED, owner="BC-001",
        verb=OwnershipVerb.OWNS, rule="declaration",
...
def test_entities_are_sorted():
    rep = _report(
        EntityOwnership(entity="zebra", outcome=OwnershipOutcome.UNCLAIMED,
                        rule="none"),
        EntityOwnership(entity="apple", outcome=OwnershipOutcome.UNCLAIMED,
                        rule="none"),
    )
    out = consolidate(rep, [])
...
def test_consolidation_is_order_independent():
    """NFR-10."""
    rows = (
        EntityOwnership(entity="a", outcome=OwnershipOutcome.UNCLAIMED,
                        rule="none"),
        EntityOwnership(entity="b", outcome=OwnershipOutcome.UNCLAIMED,
                        rule="none"),
    )
    assert (consolidate(_report(*rows), []).model_dump_json()
