...
from sdlc.activities import (
    LintInput, QAInput, _ensure_py_launcher_versions, _PY_LAUNCHER_VERSION_RE,
    run_lint, run_test_suite,
)
...
def test_regex_extracts_multiple_distinct_versions():
    assert set(_PY_LAUNCHER_VERSION_RE.findall(
        "py -3.11 -m pytest && py -3.9 -m mypy .")) == {"3.11", "3.9"}
...
def test_run_test_suite_does_not_pollute_issues_on_a_passing_run(
    ...
    report = asyncio.run(run_test_suite(QAInput(
        worktree=str(tmp_path), test_cmd=_py_cmd("pass"), timeout_s=30)))

    assert report.tests_passed is True
    assert report.issues == []


def test_run_test_suite_surfaces_provisioning_note_on_a_failing_run(
        monkeypatch, tmp_path):
    async def fake_ensure(cmd, cwd):
        return "py -3.11 was not installed; auto-provisioned"
    monkeypatch.setattr(activities, "_ensure_py_launcher_versions", fake_ensure)
    ...
    report = asyncio.run(run_test_suite(QAInput(
        worktree=str(tmp_path), test_cmd=_py_cmd("import sys; sys.exit(1)"),
        timeout_s=30)))
...
def test_run_lint_surfaces_provisioning_note_only_on_failure(
        monkeypatch, tmp_path):
    async def fake_ensure(cmd, cwd):
        return "py -3.11 was not installed; auto-provisioned"
    monkeypatch.setattr(activities, "_ensure_py_launcher_versions", fake_ensure)

    clean, detail = asyncio.run(run_lint(LintInput(
        worktree=str(tmp_path), lint_cmd=_py_cmd("pass"), timeout_s=30)))
    assert clean is True
    assert "auto-provisioned" not in detail

    clean, detail = asyncio.run(run_lint(LintInput(
        worktree=str(tmp_path), lint_cmd=_py_cmd("import sys; sys.exit(1)"),
        timeout_s=30)))
