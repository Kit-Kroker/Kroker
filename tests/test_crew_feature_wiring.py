...
def _cfg(benchmark: bool = False):
    ...
    cfg.roles["dev"] = RoleConfig(
        harness=HarnessKind.CREW, layout="code", model="zai-coding-plan/glm-5.3"
    )
    if benchmark:
        cfg.benchmark = BenchmarkConfig(case_id="crew-wiring", bench_run_id="crew-wiring-1")
    return cfg
...
async def _finish(handle, slice_s: float = 5.0, rounds: int = 30):
    ...
    for _ in range(rounds):
        try:
            return await asyncio.wait_for(asyncio.shield(handle.result()), timeout=slice_s)
        except TimeoutError:
            continue
    raise AssertionError("workflow did not finish; likely stuck in workflow-task retries")


async def test_the_crew_dev_task_carries_the_protocol_to_the_round_brief(tmp_path, monkeypatch):
    BRIEFS.clear()
    ...
    async with await WorkflowEnvironment.start_time_skipping(
        ...
        async with Worker(
            ...
            handle = await env.client.start_workflow(
                FeatureWorkflow.run,
                args=[greenfield_idea(), _cfg(), None],
...
async def test_the_code_stage_record_carries_the_attempts_token_counts(tmp_path, monkeypatch):
