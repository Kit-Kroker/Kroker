...
def test_run_state_defaults_to_an_unpriced_run():
    s = RunState(run_id="feature-x", title="X", mode="greenfield",
...
def test_run_state_carries_decisions_and_roles():
    d = GateDecision(gate="architecture", round=1,
                     outcome=GateOutcome.APPROVE, decided_by="human")
    u = RoleUsage(role="architect", model="m", calls=1, cost_usd=0.5)
    s = RunState(run_id="r", title="T", mode="brownfield", status="running",
                 started_at=AT, decisions=[d], roles=[u],
                 cost_usd_total=0.5, budget_usd=40.0)
...
def test_run_state_mirrors_run_summary_field_names_where_they_overlap():
    ...
    shared = {"run_id", "mode", "started_at", "cost_usd_total",
              "budget_usd", "budget_crossings", "roles", "title", "repo_url"}
...
def test_run_summary_carries_title_and_repo_url_for_closed_runs():
    ...
    s = RunSummary(run_id="feature-add-sso", mode="brownfield",
                   outcome="deployed:ok", terminal_stage="retro",
                   started_at=AT, ended_at=AT, duration_s=1.0,
                   title="Add SSO", repo_url="git@example:acme/portal")
...
def test_run_summary_title_defaults_empty_so_existing_callers_are_unaffected():
    s = RunSummary(run_id="r", mode="greenfield", outcome="done",
