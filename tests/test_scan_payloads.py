...
from sdlc.assessment.scan.models import (          # noqa: E402
    CiStageRecord, CoverageRecord, EnvironmentRecord, PAYLOAD_FIELD,
    ScanSignalId, SecurityObservation, TestFileRecord, TestLevel,
    security_identity,
)
...
def test_a_security_observation_declares_which_signal_owns_it():
    ...
    o = SecurityObservation(
        signal=ScanSignalId.SS1, category="tls_enforcement",
        rule="ss1_tls_verification_disabled", detail="verify=False",
        severity_hint="high", path="src/client.py", line=12,
        evidence="requests.get(url, verify=False)", key="abc123",
...
def test_a_security_observation_must_name_a_category_its_signal_owes():
    ...
    with pytest.raises(ValidationError):
        SecurityObservation(
            signal=ScanSignalId.SS1, category="db_security",   # SS3's
            rule="r", detail="d", severity_hint="low", path="p",
            confidence=Confidence.LOW)


def test_security_identity_ignores_line_like_its_two_siblings():
    o = SecurityObservation(
        signal=ScanSignalId.SS3, category="exposed_ports", rule="r",
        detail="d", severity_hint="info", path="Dockerfile", line=3,
        key="K", confidence=Confidence.LOW)
    assert security_identity(o) == security_identity(
        o.model_copy(update={"line": 99}))
...
def test_an_unclassifiable_test_file_is_unknown_never_unit():
    ...
    r = TestFileRecord(path="tests/weird.py", level=TestLevel.UNKNOWN,
                       rule="qs1_no_level_signature", mapping_rule="unmapped",
                       confidence=Confidence.LOW)
...
def test_an_unmapped_test_covers_nothing_and_a_mapped_one_covers_something():
    with pytest.raises(ValidationError):
        TestFileRecord(path="t.py", level=TestLevel.UNIT, rule="r",
                       mapping_rule="unmapped", covers=["src/a.py"],
                       confidence=Confidence.LOW)
    with pytest.raises(ValidationError):
        TestFileRecord(path="t.py", level=TestLevel.UNIT, rule="r",
                       mapping_rule="naming_convention", covers=[],
                       confidence=Confidence.LOW)
...
def test_a_proxy_coverage_record_is_low_confidence_by_construction():
    ...
    with pytest.raises(ValidationError):
        CoverageRecord(scope="package", path="src/app",
                       covered=Measurement.measured(80.0), source="proxy",
                       confidence=Confidence.HIGH)
    ok = CoverageRecord(scope="package", path="src/app",
                        covered=Measurement.measured(80.0), source="proxy",
                        confidence=Confidence.LOW)
    assert ok.tool == ""
...
def test_a_report_coverage_record_must_name_its_tool():
    ...
    with pytest.raises(ValidationError):
        CoverageRecord(scope="file", path="src/a.py",
                       covered=Measurement.measured(50.0), source="report",
                       confidence=Confidence.HIGH)
...
def test_a_ci_stage_records_blocking_as_unreadable_at_a_commit():
    ...
    s = CiStageRecord(
        workflow=".github/workflows/ci.yml", stage="test", order=0,
        runs_tests=True, test_levels=[TestLevel.UNIT],
        blocking=Measurement.not_collected(
            "required checks are a branch-protection setting, not a "
            "tracked file"))
...
def test_an_environment_must_be_declared_somewhere():
    ...
    assert EnvironmentRecord(name="prod", in_ci=True,
                             in_config=True).drifted is False
