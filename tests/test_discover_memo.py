...
KEY = dict(project="acme", tree_hash="t" * 40, context_digest="d" * 64,
...
def _ctx(candidate_id="C-01") -> DiscoverContext:
    member = CandidateMember(kind=MemberKind.HTTP_ROUTE, value="POST /pay",
                              path="pay/api.py")
    return DiscoverContext(
        candidates=(CandidateContext(
            candidate_id=candidate_id, name="payments",
            confidence=Confidence.HIGH, sources=("S3-payments",),
            source_rules=("s3_http_route",), members=(member,),
            member_paths=("pay/api.py",),
            cohesion=Measurement.measured(1.0),
            coupling=Measurement.measured(0.0), guardrail_only=False),),
        graph=GRAPH, collected=Measurement.measured(1.0))
...
def test_a_prompt_or_model_change_invalidates():
    memo.store(**KEY, registry_version=1, out=MAP)
    assert memo.load(**{**KEY, "prompt_sha": "abc"},
                     registry_version=1) is None
    assert memo.load(**{**KEY, "model": "claude-x"},
                     registry_version=1) is None
...
def test_a_not_collected_map_is_never_stored():
    ...
    forever."""
    failed = CapabilityMap(
        collected=Measurement.not_collected("S5 did not collect"))
...
def test_corrupt_content_is_a_miss_not_a_crash():
    ...
    cache.put(discover_key("acme", "t" * 40, "d" * 64, 1, NO_PROPOSER,
                           NO_PROPOSER), "{not json")
    assert memo.load(**KEY, registry_version=1) is None
