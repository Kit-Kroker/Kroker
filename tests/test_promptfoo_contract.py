...
pytestmark = pytest.mark.skipif(
    promptfoo_bin() is None, reason="promptfoo not installed (pip install -e .[eval])"
)
...
def test_config_shape_is_accepted(tmp_path):
    ...
    proc = subprocess.run(
        [
            ...
            "eval",
            "-c",
            str(tmp_path / "promptfooconfig.yaml"),
            "--output",
            str(out),
        ],
        capture_output=True,
...
def test_assertion_score_survives_into_results_json(tmp_path):
    ...
    cfg = {
        ...
        "prompts": ["{{input}}"],
        "providers": [{"id": "file://p.py:call_api", "label": "baseline", "config": {}}],
    ...
    assert scores == [0.42], f"custom assertion score was not preserved: {scores}"
