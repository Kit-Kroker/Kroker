...
from sdlc.assessment.scan.models import (
    ...
    CandidateMember, Confidence, MemberKind, ScanSignalId, ScanUpstream,
    SourceCandidate,
)
...
def _upstream(s3_ok: bool = True) -> ScanUpstream:
    if not s3_ok:
        return ScanUpstream(collected={
            ScanSignalId.S3: Measurement.not_collected("S3 failed")})
    return ScanUpstream(
        sources=[
            SourceCandidate(
                signal=ScanSignalId.S3, local_id="S3-order", name="orders",
                rule="s3_http_route", detail="d",
                confidence_contribution=Confidence.LOW,
                members=[CandidateMember(
                    kind=MemberKind.HTTP_ROUTE, value="POST /api/orders",
                    path="src/routes/orders.py", line=3)]),
            SourceCandidate(
                signal=ScanSignalId.S3, local_id="S3-payment",
                name="payments", rule="s3_http_route", detail="d",
                confidence_contribution=Confidence.LOW,
                members=[CandidateMember(
                    kind=MemberKind.HTTP_ROUTE, value="POST /api/payments",
                    path="src/routes/payments.py", line=6)]),
        ],
...
def test_disabled_certificate_verification_is_a_high_hint():
    ...
    finding = next(o for o in tls
                   if o.rule == "ss1_tls_verification_disabled")
...
def test_a_plaintext_url_is_recorded_and_localhost_is_not():
    out = security_static.evaluate(
        ...
        _upstream())
    paths = {o.path for o in out.security
             if o.rule == "ss1_plaintext_http_url"}
...
def test_an_entry_point_without_a_validation_marker_is_recorded():
    out = security_static.evaluate(BLOBS, _upstream())
    missing = [o for o in out.security
               if o.category == C_INPUT_VALIDATION]
