...
def test_apply_rejects_a_row_built_for_a_different_project(store):
    ...
    wrong = CapabilityIdentity(
        bc_id="BC-001", project="other", first_seen_run="r0", fingerprint=_fp(contract=["POST /a"])
    )
...
def test_apply_accepts_rows_whose_project_matches(store):
    right = CapabilityIdentity(
        bc_id="BC-001", project="p", first_seen_run="r0", fingerprint=_fp(contract=["POST /a"])
    )
