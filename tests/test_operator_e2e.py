...
from sdlc.models import (IdeaBrief, MemoryConfig, PipelineConfig,
                         ProjectMode)
...
async def test_verbs_answer_against_a_live_run():
    ...
    async with await WorkflowEnvironment.start_local(
        ...
        async with Worker(env.client, task_queue="chat-e2e",
                          workflows=[FeatureWorkflow], activities=[]):
            handle = await env.client.start_workflow(
                FeatureWorkflow.run,
                args=[IdeaBrief(title="Add SSO", description="d",
                                mode=ProjectMode.GREENFIELD),
                      PipelineConfig(memory=MemoryConfig(enabled=False)), None],
                id="feature-add-sso", task_queue="chat-e2e")
            ...
            deps = OperatorDeps(poller=poller, board=None, starter=None,
                                actor="chat:test")
            ...
            try:
                ...
                report = await tools.follow(deps, run_id="feature-add-sso",
                                            timeout_s=5)
