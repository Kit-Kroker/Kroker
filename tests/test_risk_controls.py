...
def _obs(
    category: str, rule: str = "r", signal: ScanSignalId = ScanSignalId.SS1
) -> SecurityObservation:
...
def test_authorization_has_no_source_and_does_not_mirror_authentication():
    rows = controls(capability(security=(_obs(C_AUTHN_AUTHZ),)), collected_categories=ALL)
...
def test_an_observation_in_a_family_marks_it_absent():
    ...
    rows = controls(capability(security=(_obs(C_INPUT_VALIDATION),)), collected_categories=ALL)
...
def test_controls_are_order_independent():
    """NFR-10."""
    obs = [_obs(C_INPUT_VALIDATION, "a"), _obs(C_TLS, "b"), _obs(C_AUTHN_AUTHZ, "c")]
    first = None
    for _ in range(5):
        random.shuffle(obs)
        rows = controls(capability(security=tuple(obs)), collected_categories=ALL)
