...
def test_an_uncollected_capability_map_yields_not_collected_never_empty():
    ...
    m = build(
        CapabilityMap(collected=Measurement.not_collected("no scan")), collected_categories=ALL
    )
...
def test_capabilities_are_sorted_by_bc_id():
    m = build(_cmap(capability("BC-002"), capability("BC-001")), collected_categories=ALL)
