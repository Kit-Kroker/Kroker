...
def _ctx(candidate_id="C-01", values=("POST /pay",), **kw):
    ...
    base = dict(
        candidate_id=candidate_id, name="payments",
        confidence=Confidence.HIGH, sources=("S3-payments",),
        source_rules=("s3_http_route",), members=members,
        member_paths=("pay/api.py",), cohesion=MEASURED, coupling=MEASURED,
        guardrail_only=False)
    return CandidateContext(**(base | kw))


def _context(*candidates) -> DiscoverContext:
    return DiscoverContext(candidates=candidates or (_ctx(),), graph=GRAPH,
...
def test_a_disposition_naming_no_candidate_is_recorded_by_id():
    ...
    got = stamp(_context(), DiscoverProposal(
        dispositions=[_prop(), _prop(candidate_id="C-99")]))
...
def test_a_merge_into_an_unknown_target_is_dropped():
    got = stamp(_context(), DiscoverProposal(dispositions=[
        _prop(action=DiscoverAction.MERGE, merge_into="C-99")]))
    assert _only(got).rule == "dropped_merge_target"


def test_a_merge_into_itself_is_dropped():
    got = stamp(_context(), DiscoverProposal(dispositions=[
        _prop(action=DiscoverAction.MERGE, merge_into="C-01")]))
...
def test_a_merge_into_a_target_that_did_not_survive_is_dropped():
    ...
    got = stamp(_context(_ctx("C-01"), _ctx("C-02"), _ctx("C-03")),
                DiscoverProposal(dispositions=[
                    _prop(candidate_id="C-01", action=DiscoverAction.MERGE,
                          merge_into="C-02"),
                    _prop(candidate_id="C-02", action=DiscoverAction.MERGE,
                          merge_into="C-03"),
...
def test_a_split_into_fewer_than_two_partitions_is_dropped():
    got = stamp(_context(), DiscoverProposal(dispositions=[
        _prop(action=DiscoverAction.SPLIT, partitions=(
...
def test_a_split_naming_a_member_the_candidate_lacks_is_dropped():
    ...
    invented members."""
    got = stamp(_context(_ctx(values=("POST /pay", "GET /pay"))),
                DiscoverProposal(dispositions=[
                    _prop(action=DiscoverAction.SPLIT, partitions=(
                        SplitPartition(name="a", member_values=("POST /pay",)),
                        SplitPartition(name="b",
                                       member_values=("DELETE /invented",)),
                    ))]))
...
def test_a_split_with_overlapping_partitions_is_dropped():
    ...
    got = stamp(_context(_ctx(values=("POST /pay", "GET /pay"))),
                DiscoverProposal(dispositions=[
                    _prop(action=DiscoverAction.SPLIT, partitions=(
                        SplitPartition(name="a",
                                       member_values=("GET /pay", "POST /pay")),
...
def test_a_malformed_disposition_is_dropped_rather_than_raising():
    ...
    got = stamp(_context(), DiscoverProposal(dispositions=[
        _prop(action=DiscoverAction.CONFIRM, merge_into="C-02")]))
