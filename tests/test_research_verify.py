...
def test_replacement_char_standing_in_for_an_apostrophe_still_grounds(runs_root):
    ...
    _write_page("r1", "https://x/1", "send it to owner�s phone into compatible app")
    brief = ResearchBrief(
        grounded_findings=[
            GroundedFinding(
                source_url="https://x/1",
                quote="send it to owner's phone into compatible app",
                claim="c",
            )
        ]
    )
...
def test_curly_and_straight_quotes_are_interchangeable(runs_root):
    ...
    brief = ResearchBrief(
        grounded_findings=[
            GroundedFinding(source_url="https://x/1", quote="the cat's favorite spot", claim="c")
        ]
    )
    assert verify.verify_brief(brief, "r1") == []
...
def test_apostrophe_normalization_does_not_mask_a_real_word_mismatch(runs_root):
    ...
    assert [v.kind for v in verify.verify_brief(brief, "r1")] == ["quote_not_found"]
...
def test_markdown_bold_normalization_does_not_mask_a_real_word_mismatch(runs_root):
    ...
    brief = ResearchBrief(
        grounded_findings=[
            GroundedFinding(
                source_url="https://x/1",
                quote="UWB can achieve centimeter-level precision",
                claim="c",
            )
        ]
    )
