...
def _sc(
    ...
    return SourceCandidate(
        ...
        members=members
        or [CandidateMember(kind=MemberKind.PACKAGE_PATH, value=f"src/{slug}", path=f"src/{slug}")],
    )
...
def test_overlapping_members_under_different_names_are_not_collapsed():
    ...
    shared = CandidateMember(
        kind=MemberKind.FILE_PATH, value="src/billing/core.py", path="src/billing/core.py"
    )
    out = merge(
        [
...
def test_two_groups_sharing_a_file_path_are_flagged_as_duplicates():
    ...
    file = CandidateMember(
        kind=MemberKind.FILE_PATH, value="src/billing/core.py", path="src/billing/core.py"
    )
    route = CandidateMember(
        kind=MemberKind.HTTP_ROUTE, value="POST /refund", path="src/billing/core.py", line=12
    )
    out = merge(
        [
            SourceCandidate(
