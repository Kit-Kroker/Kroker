...
def test_workflow_id_is_per_run_not_per_repository():
    ...
    now = datetime(2026, 8, 9, 10, 15, 0, tzinfo=UTC)
...
def test_two_ids_for_one_repo_differ():
    a = tidyup_workflow_id("/x/r", datetime(2026, 8, 9, 10, 15, 0, tzinfo=UTC))
    b = tidyup_workflow_id("/x/r", datetime(2026, 8, 9, 10, 16, 0, tzinfo=UTC))
    assert a != b
