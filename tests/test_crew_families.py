...
def test_preflight_refuses_a_colliding_lead_model(tmp_path, monkeypatch):
    ...
    (tmp_path / "layouts" / "code.yaml").write_text(yaml.safe_dump({
        "layout": "code", "lead": "coder", "crew": ["coder", "critic"],
        "rounds": {"max": 2},
        "deliverable": {"path": "notes.md", "schema": "notes-v1"},
        "limits": {"wall_clock_s": 1, "turn_timeout_s": 1, "cost_usd": 1.0},
    }), encoding="utf-8")
    (tmp_path / "roles" / "coder.yaml").write_text(yaml.safe_dump({
        "harness": "opencode", "model": "zai-coding-plan/glm-5.3",
        "writes": True, "skill": "coder"}), encoding="utf-8")
    (tmp_path / "roles" / "critic.yaml").write_text(yaml.safe_dump({
        "harness": "claude_code", "model": "anthropic:claude-opus-5",
        "writes": False, "skill": "critic"}), encoding="utf-8")
