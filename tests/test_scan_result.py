...
from sdlc.assessment.scan.models import (
    CATEGORIES, CandidateMember, Confidence, MemberKind, SCAN_ORDER,
    ScanCandidate, ScanResult, ScanSignalId, ScanSignalResult, SignalOutput,
...
def _row(sid: ScanSignalId, measured: bool = True) -> ScanSignalResult:
    val = (Measurement.measured(0.0) if measured
           else Measurement.not_collected(f"{sid.value} stub"))
...
def _candidate() -> ScanCandidate:
    return ScanCandidate(
        candidate_id="C-01", name="Payments", sources=["S1-pay", "S3-pay"],
        confidence=Confidence.MEDIUM,
        members=[CandidateMember(kind=MemberKind.HTTP_ROUTE, value="GET /p")])


def test_signals_must_be_the_whole_set_in_order():
    r = ScanResult(signals=_all_rows(), sources=[], candidates=[],
                   data_sensitivity=[], testability=[])
...
def test_a_missing_signal_is_refused():
    with pytest.raises(ValidationError, match="whole set"):
        ScanResult(signals=_all_rows()[:-1], sources=[], candidates=[],
                   data_sensitivity=[], testability=[])
...
def test_out_of_order_signals_are_refused():
    ...
    with pytest.raises(ValidationError, match="whole set"):
        ScanResult(signals=rows, sources=[], candidates=[],
                   data_sensitivity=[], testability=[])
...
def test_a_not_measured_signal_carrying_a_payload_is_refused():
    ...
    rows = _all_rows()
    rows[SCAN_ORDER.index(ScanSignalId.S5)] = _row(ScanSignalId.S5,
                                                   measured=False)
    with pytest.raises(ValidationError, match="did not run"):
        ScanResult(signals=rows, sources=[], candidates=[_candidate()],
                   data_sensitivity=[], testability=[])
