...
def test_opencode_build_cmd_auto_approves_edits():
    ...
    cmd = OpenCodeHarness().build_cmd(
        HarnessRequest(prompt="do stuff", cwd="/tmp/wt", model="zai-coding-plan/glm-5.2")
    )
    assert "--auto" in cmd
...
def test_claude_build_cmd_accept_edits():
    ...
    cmd = ClaudeCodeHarness().build_cmd(HarnessRequest(prompt="do stuff", cwd="/tmp/wt"))
