...
from sdlc.assessment.scan.models import (
    CATEGORIES, CandidateMember, Confidence, MemberKind, ScanSignalId,
    ScanSignalResult, ScanUpstream, SignalOutput, SignalSource,
    SourceCandidate, TestFileRecord, TestLevel, family_of,
)
...
def _candidate(sid: ScanSignalId, local_id: str) -> SourceCandidate:
    return SourceCandidate(
        signal=sid, local_id=local_id, name=local_id, rule="r", detail="d",
...
def _test_file(path: str) -> TestFileRecord:
    return TestFileRecord(path=path, level=TestLevel.UNIT, rule="r",
                          mapping_rule="naming_convention",
                          covers=["src/a.py"], confidence=Confidence.MEDIUM)
...
def test_ss4_receives_both_the_signals_it_consumes():
    ...
    assert SCAN_SIGNALS[ScanSignalId.SS4].consumes == (
        ScanSignalId.S2, ScanSignalId.S3)
    measured = Measurement.measured(1.0)
    outputs = {
        ScanSignalId.S1: SignalOutput(row=_row(ScanSignalId.S1, measured),
                                      sources=[_candidate(ScanSignalId.S1, "S1-a")]),
        ScanSignalId.S2: SignalOutput(row=_row(ScanSignalId.S2, measured),
                                      sources=[_candidate(ScanSignalId.S2, "S2-orders")]),
        ScanSignalId.S3: SignalOutput(row=_row(ScanSignalId.S3, measured),
                                      sources=[_candidate(ScanSignalId.S3, "S3-orders")]),
    }
...
def test_qs2_receives_qs1s_test_records_not_candidates():
    ...
    outputs = {ScanSignalId.QS1: SignalOutput(
        row=_row(ScanSignalId.QS1, measured),
        tests=[_test_file("tests/test_a.py")])}
...
def test_a_measured_row_with_a_degraded_upstream_is_not_cached():
    ...
    degraded = ScanUpstream(collected={
        ScanSignalId.S3: Measurement.not_collected("S3 failed")})
    assert memo.store(ScanSignalId.SS1, TREE, out, degraded) is False
    assert memo.load(ScanSignalId.SS1, TREE) is None

    healthy = ScanUpstream(collected={
        ScanSignalId.S3: Measurement.measured(3.0)})
