...
from sdlc.crew.models import (
    MAX_NOTE_BYTES, NOTE_SCHEMA, CrewRunResult, RoundNote, RoundRecord,
    TurnBeat, TurnRecord,
)
...
def _note(**kw):
    base = dict(schema=NOTE_SCHEMA, what_changed="added greet()",
                why="the brief asked for it", verification="ran pytest")
    base.update(kw)
...
def test_turn_record_marks_a_lost_cost_rather_than_zeroing_it():
    ...
    t = TurnRecord(role="coder", round=1, attempt=2,
                   harness=HarnessKind.OPENCODE, model="glm-5.3",
                   session_id=None, cost_usd=None, exit_code=None,
...
def test_round_cost_sums_every_attempt_including_abandoned_ones():
    ...
    r = RoundRecord(round=1, turns=[
        TurnRecord(role="coder", round=1, attempt=1,
                   harness=HarnessKind.OPENCODE, model="glm-5.3",
                   cost_usd=0.40),
        TurnRecord(role="coder", round=1, attempt=2,
                   harness=HarnessKind.OPENCODE, model="glm-5.3",
                   cost_usd=0.25),
    ])
...
def test_crew_result_carries_the_lead_session_on_the_shared_contract():
    ...
    run = HarnessRunResult(harness=HarnessKind.OPENCODE, exit_code=0,
                           summary="done", session_id="s-lead")
