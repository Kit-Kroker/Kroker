...
from sdlc.assessment.scan.models import (
    C_DATA_SENSITIVITY, C_ENTITY_ACCESS, CandidateMember, Confidence,
    MemberKind, ScanSignalId, ScanUpstream, Sensitivity, SourceCandidate,
)
...
def _upstream(s2_ok=True, s3_ok=True) -> ScanUpstream:
    ...
    if s2_ok:
        ...
        sources.append(SourceCandidate(
            signal=ScanSignalId.S2, local_id="S2-customer", name="customers",
            rule="s2_schema_cluster", detail="d",
            confidence_contribution=Confidence.LOW,
            members=[CandidateMember(kind=MemberKind.DB_TABLE,
                                     value="customers")]))
    else:
    ...
    if s3_ok:
        ...
        sources.append(SourceCandidate(
            signal=ScanSignalId.S3, local_id="S3-customer", name="customer",
            rule="s3_http_route", detail="d",
            confidence_contribution=Confidence.LOW,
            members=[CandidateMember(kind=MemberKind.HTTP_ROUTE,
                                     value="GET /api/customers")]))
    else:
...
def test_pii_and_authentication_are_distinct_classifications():
    ...
    pii = next(r for r in out.data_sensitivity
               if r.classification is Sensitivity.PII)
...
def test_a_financial_model_is_classified_from_its_fields():
    out = sensitivity.evaluate(BLOBS, _upstream())
    fin = next(r for r in out.data_sensitivity
               if r.classification is Sensitivity.FINANCIAL)
...
def test_entity_access_is_a_gap_when_s3_did_not_collect():
    ...
    assert out.row.categories[C_ENTITY_ACCESS].state is \
    ...
    assert out.row.categories[C_DATA_SENSITIVITY].state is \
...
def test_a_compliance_marker_makes_the_entity_regulatory():
    ...
    out = sensitivity.evaluate(blobs, _upstream())
    assert any(r.classification is Sensitivity.REGULATORY
               and r.entity == "cards" for r in out.data_sensitivity)
...
def test_output_is_byte_identical_across_input_orderings():
    ...
    assert sensitivity.evaluate(reordered,
                                _upstream()).model_dump_json() == reference
