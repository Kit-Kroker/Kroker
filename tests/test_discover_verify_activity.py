...
from sdlc.assessment.discover.map import (
    DiscoverAction, DiscoverProposal, EvidenceRef, ProposedDisposition,
)
...
def _proposal(**kw):
    return DiscoverProposal(dispositions=[ProposedDisposition(
...
async def test_a_real_path_resolves_against_the_pinned_commit(repo):
    repo_dir, sha = repo
    out = await verify_discover_refs(VerifyRefsInput(
...
async def test_a_directory_path_does_not_resolve_as_a_file(repo):
    ...
    out = await verify_discover_refs(VerifyRefsInput(
        repo_dir=repo_dir, commit_sha=sha,
        proposal=_proposal(evidence=(EvidenceRef(path="."),))))
    assert out.refusals["C1"][0] == "dropped_ref_unresolved"
...
async def test_a_quote_byte_verifies_against_the_committed_bytes(repo):
    ...
    out = await verify_discover_refs(VerifyRefsInput(
        repo_dir=repo_dir, commit_sha=sha,
        proposal=_proposal(evidence=(EvidenceRef(path="pay.py", lines="1-2"),),
                           quote="gateway.charge")))
    assert out.refusals == {}


@pytest.mark.asyncio
async def test_an_unreadable_repo_refuses_every_reference_rather_than_raising(
        tmp_path):
