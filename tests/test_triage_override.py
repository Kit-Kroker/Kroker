...
def test_override_records_the_class_of_approver_verbatim():
    ...
    o = ReadinessOverride(approved_by="policy", reason="gate off",
...
def test_approved_by_rejects_a_principal():
    ...
    with pytest.raises(Exception):
        ReadinessOverride(approved_by="alice", reason="r",
                          decided_at=dt.datetime(2026, 8, 8,
                                                 tzinfo=dt.timezone.utc),
                          gate_round=1)
...
def test_reviewer_carries_the_self_asserted_identity():
    ...
    o = ReadinessOverride(approved_by="human", reviewer="alice", reason="ok",
                          decided_at=dt.datetime(2026, 8, 8,
                                                 tzinfo=dt.timezone.utc),
                          gate_round=1)
