...
def _harness_cell(model, harness=HarnessKind.OPENCODE, arm_name="a"):
    ...
    return BenchmarkCell(case_id="todo-api", harness=harness, arm_name=arm_name,
                         role_models={"dev": model, "test": model,
                                      "devops": model})
...
def test_cell_config_forwards_per_model_extra_args():
    ...
    cfg = _cell_config(base, idea, spec,
    ...
    cfg2 = _cell_config(base, idea, _spec(), _harness_cell("openai/gpt-5.2"),
                        bench_run_id="b1")
...
def test_cell_config_enables_research_and_injects_provider():
    base = PipelineConfig()
    idea = IdeaBrief(title="t", description="d", mode=ProjectMode.GREENFIELD)
    cfg = _cell_config(base, idea, _research_spec(),
                       _harness_cell("zai-coding-plan/glm-5.2"),
                       bench_run_id="b1")
