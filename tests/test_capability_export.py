...
from sdlc.capability.export import (
    build_export, fingerprint_sha256, write_export,
)
...
def test_hash_is_stable_across_member_order():
    assert fingerprint_sha256(_fp("POST /a", "POST /b")) == \
        fingerprint_sha256(_fp("POST /b", "POST /a"))


def test_hash_changes_when_a_member_changes():
    assert fingerprint_sha256(_fp("POST /a")) != \
...
def test_export_entries_are_sorted_by_bc_id():
    payload = build_export("p", [_identity("BC-002", _fp("POST /b")),
                                 _identity("BC-001", _fp("POST /a"))])
    assert [e["bc_id"] for e in payload["capabilities"]] == \
        ["BC-001", "BC-002"]


def test_export_records_status_and_merge_target():
    payload = build_export("p", [
        _identity("BC-001", _fp("POST /a"), status=IdentityStatus.MERGED,
                  merged_into="BC-002")])
...
def test_retired_entries_are_exported_so_delivered_refs_resolve():
    payload = build_export("p", [
        _identity("BC-001", _fp("POST /a"), status=IdentityStatus.RETIRED,
