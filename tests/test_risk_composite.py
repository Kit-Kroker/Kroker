...
def test_no_factor_collected_carries_no_drivers():
    c = Composite(
        value=Measurement.not_collected("nothing collected"), factors=(_f("a", None), _f("b", None))
    )
    assert c.is_partial is False
    assert c.drivers == ()
...
def test_a_measured_composite_needs_every_factor_collected():
    with pytest.raises(ValidationError, match="did not collect"):
        Composite(value=Measurement.measured(0.4), factors=(_f("a", 0.2), _f("b", None)))


def test_a_driver_must_name_a_factor_that_exists():
    with pytest.raises(ValidationError, match="names no factor"):
        Composite(
            value=Measurement.measured(0.2),
            factors=(_f("a", 0.2),),
            drivers=(Driver(factor_key="zzz", value=0.1, contribution=0.1),),
        )
...
def test_at_most_three_drivers():
    ...
    with pytest.raises(ValidationError, match="at most three"):
        Composite(
            value=Measurement.measured(0.5),
            factors=factors,
            drivers=tuple(Driver(factor_key=k, value=0.5, contribution=0.1) for k in "abcd"),
        )


def test_factors_are_asserted_sorted_not_repaired():
    with pytest.raises(ValidationError, match="sorted"):
        Composite(value=Measurement.measured(0.4), factors=(_f("b", 0.6), _f("a", 0.2)))


def test_is_partial_is_not_settable():
    with pytest.raises(ValidationError):
        Composite(value=Measurement.measured(0.2), factors=(_f("a", 0.2),), is_partial=True)
