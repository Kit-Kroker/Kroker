...
from sdlc.activities import (
    CodingTaskInput, HarnessRunResult, QAInput, evaluate_gate,
)
from sdlc.models import (
    GateConfig, GateDecision, GateOutcome, GatePolicy, QAReport,
)
...
async def scripted_test_suite(inp: QAInput) -> QAReport:
    if SCRIPT.qa_failures > 0:
        ...
        return QAReport(tests_passed=False, failing_tests=["test_hello"],
                        issues=["assert 500 == 200"])
...
async def test_task_gate_revise_retries_with_the_operator_guidance(
    ...
    async def driver(handle):
        ...
        await handle.signal(
            ...
            GateDecision(gate=TASK_GATE, round=1, outcome=GateOutcome.REVISE,
                         decided_by="human",
    ...
    assert SCRIPT.coding_calls == 3, (
        f"expected a third coding attempt after REVISE, "
        f"got {SCRIPT.coding_calls}")
    assert any("revert the shared fixture edits" in p
...
async def test_task_gate_reject_still_quarantines(tmp_path, monkeypatch):
    ...
    async def driver(handle):
        ...
        await handle.signal(
            FeatureWorkflow.submit_gate_decision,
            GateDecision(gate=TASK_GATE, round=1, outcome=GateOutcome.REJECT,
                         decided_by="human"))
...
async def test_task_gate_revise_is_bounded_by_max_gate_rounds(
        tmp_path, monkeypatch):
    ...
    async def driver(handle):
        ...
        for round in (1, 2):
            ...
            await handle.signal(
                FeatureWorkflow.submit_gate_decision,
                GateDecision(gate=TASK_GATE, round=round,
        ...
        await handle.signal(
            FeatureWorkflow.submit_gate_decision,
            GateDecision(gate=TASK_GATE, round=3, outcome=GateOutcome.REJECT,
                         decided_by="human"))

    result = await _run(_cfg(max_gate_rounds=2), tmp_path, monkeypatch,
                        "tgr3", driver)
    assert result == "failed:quarantined-tasks", result
