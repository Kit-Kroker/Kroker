...
def _ctx(cases_root, agents_dir, case="cat-cafe-monitoring", role="clarify"):
    return {"vars": {"role": role, "case": case,
...
def test_load_vetoes_reads_the_registered_file(repo_cases_root):
    ...
    assert {v.id for v in vetoes} == {"scope_preserved",
                                      "scope_discipline_declared"}


def test_load_vetoes_returns_empty_for_an_unregistered_case_role(
        repo_cases_root):
    """No vetoes registered is NOT an error -- vetoes are opt-in per case."""
    assert load_vetoes("add-login-greenfield", "clarify",
                       repo_cases_root) == []


def test_complete_artifact_passes(repo_cases_root, repo_agents_dir):
    r = get_assert(json.dumps(_GOOD),
                   _ctx(repo_cases_root, repo_agents_dir))
...
def test_output_type_failure_still_wins_over_vetoes(
        repo_cases_root, repo_agents_dir):
