...
ROLES = {"coder": CrewRole(name="coder", harness="opencode",
                           model="zai-coding-plan/glm-5.3", writes=True,
                           skill="coder")}


def test_role_config_carries_the_crew_knobs():
    rc = RoleConfig(harness=HarnessKind.CREW, layout="code",
...
def test_the_run_model_wins_for_the_lead():
    out = resolve_crew_roles(LAYOUT, ROLES, lead_harness=None,
                             lead_model="zai-coding-plan/glm-5.9")
...
def test_the_run_harness_wins_for_the_lead():
    out = resolve_crew_roles(LAYOUT, ROLES,
                             lead_harness=HarnessKind.CLAUDE_CODE,
...
def test_a_harness_swap_without_a_model_is_refused():
    ...
    with pytest.raises(ValueError, match="model"):
        resolve_crew_roles(LAYOUT, ROLES,
                           lead_harness=HarnessKind.CLAUDE_CODE,
                           lead_model=None)
...
def test_the_lead_may_not_resolve_to_crew():
    with pytest.raises(ValueError, match="not a CLI"):
        resolve_crew_roles(LAYOUT, ROLES, lead_harness=HarnessKind.CREW,
                           lead_model="x")
