...
from sdlc.board.store import (BoardStore, ConflictError, InvalidTransition,
                              NotFoundError)
...
def _tasks() -> list[DevTask]:
    return [
        DevTask(id="T01", title="config", description="d",
                acceptance_criteria=["a"]),
        DevTask(id="T02", title="api", description="d",
                acceptance_criteria=["a"], depends_on=["T01"]),
    ]
...
@pytest.fixture
def plan_v(store):
    _, vid = store.publish_artifact_version(
        "proj", "plan", "run-1", b"{}", actor="workflow:run-1")
    store.sync_plan_tasks("proj", vid, "run-1", _tasks(),
...
def test_sync_is_idempotent(store, plan_v):
    n = store.sync_plan_tasks("proj", plan_v, "run-1", _tasks(),
                              actor="workflow:run-1")
...
def test_workflow_write_moves_both_statuses(store, plan_v):
    t = store.set_task_authoritative(
        "proj", plan_v, "T01", TaskStatus.IN_PROGRESS,
        actor="workflow:run-1")
...
def test_agent_write_does_not_move_authoritative_status(store, plan_v):
    ...
    t = store.set_task_observational(
        "proj", plan_v, "T01", TaskStatus.IN_PROGRESS,
        actor="agent:worker-a", expect_row_version=before.row_version)
    assert t.status is TaskStatus.IN_PROGRESS
...
def test_stale_row_version_is_a_conflict(store, plan_v):
    ...
    store.set_task_observational(
        "proj", plan_v, "T01", TaskStatus.IN_PROGRESS,
        actor="agent:a", expect_row_version=before.row_version)
    with pytest.raises(ConflictError):
        store.set_task_observational(
            "proj", plan_v, "T01", TaskStatus.BLOCKED,
            actor="agent:b", expect_row_version=before.row_version)


def test_invalid_transition_is_rejected(store, plan_v):
    with pytest.raises(InvalidTransition):
        store.set_task_authoritative(
            "proj", plan_v, "T01", TaskStatus.DONE, actor="workflow:run-1")
...
def test_rejected_write_appends_no_event(store, plan_v):
    ...
    assert len(store.list_events("proj")) == before, \
        "the change log records real changes only"
...
def test_authoritative_validates_against_authoritative_status(store, plan_v):
    ...
    before = store.get_task("proj", plan_v, "T01")
    store.set_task_observational("proj", plan_v, "T01",
                                 TaskStatus.IN_PROGRESS, actor="agent:a",
...
def test_fix_attempts_and_error_are_recorded(store, plan_v):
    store.set_task_authoritative("proj", plan_v, "T01",
                                 TaskStatus.IN_PROGRESS, actor="workflow:r")
    t = store.set_task_authoritative(
        "proj", plan_v, "T01", TaskStatus.FAILED, actor="workflow:r",
        fix_attempts=2, error="build failed", branch="task/T01")
    assert (t.fix_attempts, t.error, t.branch) == (2, "build failed",
                                                   "task/T01")
...
def test_two_threads_claiming_one_task_yield_one_winner(tmp_path):
    ...
    _, vid = setup.publish_artifact_version("proj", "plan", "r", b"{}",
                                            actor="workflow:r")
    ...
    def claim(name: str) -> None:
        ...
        try:
            s.set_task_observational("proj", vid, "T01",
                                     TaskStatus.IN_PROGRESS,
                                     actor=f"agent:{name}",
