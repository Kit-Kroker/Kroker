...
from sdlc.board.activities import (AttachEvidenceInput, PublishArtifactInput,
                                   SetTaskStatusInput, SyncPlanTasksInput,
...
ACTIVITIES = [publish_artifact_version, sync_plan_tasks,
...
def test_registered_on_the_worker(name):
    ...
    assert name in inspect.getsource(worker), \
        f"{name} missing from worker registration"
...
async def test_publish_then_sync_then_transition(board_env):
    pub = await publish_artifact_version(PublishArtifactInput(
        project="proj", key="plan", run_id="run-1",
        content_json='{"tasks":[]}', actor="workflow:run-1"))
    assert pub.version_id > 0
    assert pub.ref.sha256

    n = await sync_plan_tasks(SyncPlanTasksInput(
        project="proj", plan_version=pub.version_id, run_id="run-1",
        tasks=[DevTask(id="T01", title="a", description="d",
                       acceptance_criteria=["x"])],
        actor="workflow:run-1"))
    assert n == 1

    await set_task_authoritative(SetTaskStatusInput(
        project="proj", plan_version=pub.version_id, task_id="T01",
        status=TaskStatus.IN_PROGRESS, actor="workflow:run-1"))

    ref = await attach_task_evidence(AttachEvidenceInput(
        project="proj", plan_version=pub.version_id, task_id="T01",
        run_id="run-1", kind="qa", content_json='{"passed":true}'))
...
async def test_activities_are_idempotent_under_retry(board_env):
    ...
    pub = await publish_artifact_version(PublishArtifactInput(
        project="proj", key="plan", run_id="run-1", content_json="{}",
        actor="workflow:run-1"))
