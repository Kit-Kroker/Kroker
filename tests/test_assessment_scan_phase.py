...
def test_collected_from_categories_is_not_collected_when_any_category_unmeasured():
    cats = {"a": Measurement.measured(1.0), "b": Measurement.not_collected("triage timed out")}
    m = _collected_from_categories(cats)
...
def _candidate(signal: ScanSignalId, local_id: str) -> SourceCandidate:
    kind = MemberKind.PACKAGE_PATH if signal is ScanSignalId.S1 else MemberKind.HTTP_ROUTE
...
def test_upstream_for_returns_only_declared_consumes_candidates():
    ...
    outputs = {
        ...
        ScanSignalId.S2: SignalOutput(
            row=_measured_row(ScanSignalId.S2), sources=[_candidate(ScanSignalId.S2, "S2-orders")]
        ),
        ScanSignalId.S3: SignalOutput(
            row=_measured_row(ScanSignalId.S3), sources=[_candidate(ScanSignalId.S3, "S3-pay")]
        ),
        ScanSignalId.S4: SignalOutput(
            row=_measured_row(ScanSignalId.S4), sources=[_candidate(ScanSignalId.S4, "S4-shop")]
        ),
    }
