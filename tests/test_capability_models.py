...
from sdlc.capability.models import (
    ...
    CapabilityIdentity, DEFAULT_TIER_WEIGHTS, IdentityAttachment,
    IdentityStatus, ProposedCapability, ResolutionResult, RetiredReason,
    SignalTier,
)
...
def test_retired_identity_requires_a_reason():
    with pytest.raises(ValidationError, match="retired_reason"):
        CapabilityIdentity(
            bc_id="BC-001", project="p", first_seen_run="r",
            status=IdentityStatus.RETIRED, fingerprint=_fp(contract=["a"]))
...
def test_merged_identity_requires_merged_into():
    with pytest.raises(ValidationError, match="merged_into"):
        CapabilityIdentity(
            bc_id="BC-001", project="p", first_seen_run="r",
            status=IdentityStatus.MERGED, fingerprint=_fp(contract=["a"]))
...
def test_merged_into_must_not_be_self():
    with pytest.raises(ValidationError, match="itself"):
        CapabilityIdentity(
            bc_id="BC-001", project="p", first_seen_run="r",
            status=IdentityStatus.MERGED, merged_into="BC-001",
            fingerprint=_fp(contract=["a"]))
...
def test_first_discovery_attachment_carries_no_score():
    with pytest.raises(ValidationError, match="match_score"):
        IdentityAttachment(
            local_key="c0", bc_id="BC-001",
            method=AttachMethod.FIRST_DISCOVERY, match_score=0.9)


def test_matched_attachment_requires_a_score():
    with pytest.raises(ValidationError, match="match_score"):
        IdentityAttachment(
            local_key="c0", bc_id="BC-001", method=AttachMethod.MATCHED)
...
def test_advisory_carries_kind_and_detail():
    a = Advisory(kind=AdvisoryKind.POSSIBLE_RENAME, detail="near miss",
                 related_bc_id="BC-002", score=0.51)
