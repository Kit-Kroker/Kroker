...
- [X] T009 Implement `PipelineGraph.document_sha()` in `src/sdlc/graph/model.py`: sha256 of `_dumps(model_dump(mode="json", exclude_defaults=True))` with cosmetics kept, placed beside `content_sha` (R-6). Command: `pytest tests/graph/test_graph_content_sha.py`.
- [X] T010 RED tests in the new `tests/graph/test_graph_store_e77.py` (keeps `test_graph_store.py` small), all with a `tmp_path` root:
...
- [X] T015 [P] [US1] RED unit tests in the new `tests/graph_workflow/test_record_stamping.py` (pure, no Temporal: build `GraphWorkflow`, set `_graph_sha` and a stub dispatcher with `_attrib`, and drive `_stamp` with `ACTIVATION` set or unset):
...
- [X] T024 [US1] Add ownership rows to `src/sdlc/workflows/AGENTS.md`:
...
- [X] T028 [US2] RED→GREEN test in `tests/test_benchmark_heatmap.py`: records with `stage="unknown"` render in the trailing non-canonical bucket, before `oracle` (existing behaviour, now pinned for E-77). Command: `pytest tests/test_benchmark_heatmap.py`.
...
- [X] T029 [P] [US3] RED tests in the new `tests/graph/test_fail_reentry.py`, with a fixture registry built in the test: the shipped types plus an injected `fixer` stage type with `_in("failure", "NodeFailure")`, and a graph whose `code`-like node's `fail` port routes over a back edge (`max_traversals: 3`) into it:
...
- [X] T033 [US3] Implement the once-per-activation pass in `build_heatmap` in `src/sdlc/benchmarks/heatmap.py` (R-12). The per-record loop is unchanged; the pass is skipped when no indicator exists. Command: `pytest tests/test_benchmark_heatmap.py`.
...
- [X] T040 [US4] Regenerate the recorded fixtures with `python scripts/dump_graph_fixtures.py`. Then run `pytest tests/test_graph_fixtures_fresh.py`. Review the diff: only `catalog.json` capabilities and `run_state/graph_state.recorded.json` `canonical_stage` fields change, under `interfaces/dashboard/frontend/src/api/__fixtures__/graph/`.
- [X] T041 [US4] Test-only frontend updates forced by the flip (FR-026):
...
- [x] T044 Walk through `.specify/specs/001-canonical-stage-graph-sha/quickstart.md` §1–§3 and tick each row; the §4 manual smoke is optional. (2026-09-20: all §1 rows rc=0; §2 row 1 temporal rc=0 under the bounded wrapper, stale rows corrected in place — reentry/unknown/parent-wiring scenarios live in fast-tier files (`test_fail_reentry.py`, `test_record_stamping.py`, `test_parent_wiring.py`), replay neutrality split per-file rc=0 with `git status tests/replay` clean; §3 all green incl. dump-then-fresh with the conftest dummy keys, dump byte-identical. Corrections marked "(corrected)" in quickstart.md.)
- [x] T045 Landing docs (R5, R-13), describing main only: (2026-09-20, post-merge confirmation: main ff'd to 2ee9a87. Applied from `.workspace/tmp/e77-t045-landing-draft.md` per orchestrator rulings a–d: FR-1201 ticked with ⚠️ dropped and "Partial:"→"Landed:"; FR-1206 ticked with Landed clause naming E74-OQ-3; E-77 Landed paragraph in pipeline-as-data.md naming all three follow-ups and citing the .specify spec; ARCHITECTURE.md store line + dashboard line credit E-77; two dated errata in the E-72/E-75 superpowers specs; stale forward refs left as landing-time history.)
