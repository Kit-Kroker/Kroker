...
def _stamp_one(
    ...
    if proposed.action is DiscoverAction.SPLIT:
        ...
        if refusal_rule:
            return _dropped(
                cid, refusal_rule, "the split does not partition this candidate's own members"
            )
...
def apply(context: DiscoverContext, stamped: StampedProposal) -> ApplyResult:
    ...
    for context_row in context.candidates:
        ...
        if taken:
            sec_map = {
                (s.signal.value, s.rule, s.path, s.key): s
                for s in itertools.chain(context_row.security, *(a.security for a in taken))
            }
            sec = tuple(
                sorted(sec_map.values(), key=lambda s: (s.signal.value, s.rule, s.path, s.key))
            )
            sens_map = {
                (s.classification.value, s.entity): s
                for s in itertools.chain(context_row.sensitivity, *(a.sensitivity for a in taken))
            }
            sens = tuple(
                sorted(sens_map.values(), key=lambda s: (s.classification.value, s.entity))
            )
            testab_map = {
                (t.pattern, t.path, t.key): t
                for t in itertools.chain(context_row.testability, *(a.testability for a in taken))
            }
            testab = tuple(sorted(testab_map.values(), key=lambda t: (t.pattern, t.path, t.key)))
            cov_map = {
                c.path: c
                for c in itertools.chain(context_row.coverage, *(a.coverage for a in taken))
            }
            cov = tuple(sorted(cov_map.values(), key=lambda c: c.path))
        else:
...
def capabilities_of(applied: ApplyResult, bc_of: Mapping[str, str]) -> tuple[Capability, ...]:
    ...
    *,
    ...
    return CapabilityMap(
        ...
        by_action={
            ...
            if any(c.disposition.action is action for c in capabilities)
        },
