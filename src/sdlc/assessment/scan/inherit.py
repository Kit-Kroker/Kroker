...
from .models import (
    C_AUTHN_AUTHZ, C_CI_PRESENT, C_CREDENTIAL_STORAGE, C_DIRECT_DEPS,
    C_FRAMEWORK_DEFAULTS, C_TESTS_PRESENT, InheritedProducer, ScanSignalId,
)
...
def _absent(signal: str) -> Measurement:
    return Measurement.not_collected(
        f"triage signal {signal!r} is not present in this triage")
...
def _unavailable(sig: SignalResult) -> Measurement | None:
    ...
    if sig.collected.state is CollectionState.MEASURED:
        return None
    return Measurement.not_collected(
        f"triage signal {sig.signal!r} reported "
        f"{sig.collected.state.value}: {sig.collected.reason}")


def _tally(sig: SignalResult, rules: tuple[str, ...] | None = None
           ) -> Measurement:
    ...
    return Measurement.measured(
        float(sum(1 for f in sig.findings if f.rule in rules)))
...
def _metric(sig: SignalResult, key: str) -> Measurement:
    ...
    return sig.metrics.get(
        key, Measurement.not_collected(
            f"triage signal {sig.signal!r} reported no {key!r} metric"))


def _producer(sig: SignalResult, rules: tuple[str, ...] | None = None
              ) -> InheritedProducer:
    ...
    return InheritedProducer(
        producer=f"triage:{sig.signal}", version=sig.version,
...
def _merge_producers(*producers: InheritedProducer) -> InheritedProducer:
    ...
    return InheritedProducer(producer=names,
                             version=max(p.version for p in producers),
                             finding_ids=ids)


def inherited_halves(
        triage: RepoTriage) -> dict[ScanSignalId, InheritedHalf]:
    ...
    out[ScanSignalId.SS1] = InheritedHalf(
        producer=_merge_producers(
            _producer(secrets) if secrets
            else InheritedProducer(producer="triage:secrets", version=0),
            _producer(misconfig, _AUTHZ_RULES) if misconfig
            else InheritedProducer(producer="triage:misconfig", version=0)),
        categories={
            C_CREDENTIAL_STORAGE: (_tally(secrets) if secrets
                                   else _absent("secrets")),
            C_AUTHN_AUTHZ: (_tally(misconfig, _AUTHZ_RULES) if misconfig
                            else _absent("misconfig")),
        })

    # SS2 -- purely inherited; D12 cut transitive enumeration.
    deps = sig("dependencies")
    out[ScanSignalId.SS2] = InheritedHalf(
        producer=(_producer(deps) if deps
                  else InheritedProducer(producer="triage:dependencies",
                                         version=0)),
        categories={C_DIRECT_DEPS: (_tally(deps) if deps
                                    else _absent("dependencies"))})
    ...
    out[ScanSignalId.SS3] = InheritedHalf(
        producer=(_producer(misconfig) if misconfig
                  else InheritedProducer(producer="triage:misconfig", version=0)),
        categories={
            C_FRAMEWORK_DEFAULTS: (
                _tally(misconfig, None) if misconfig
                else _absent("misconfig")),
        })
    ...
    out[ScanSignalId.QS1] = InheritedHalf(
        producer=(_producer(baseline) if baseline
                  else InheritedProducer(producer="triage:baseline", version=0)),
        categories={
            C_TESTS_PRESENT: (_metric(baseline, "tests_present") if baseline
                              else _absent("baseline")),
        })

    # QS4 -- ci_present is the ABSENCE of baseline's no_ci finding.
    out[ScanSignalId.QS4] = InheritedHalf(
        producer=(_producer(baseline, _CI_RULES) if baseline
                  else InheritedProducer(producer="triage:baseline", version=0)),
        categories={
            C_CI_PRESENT: (_absence_of(baseline, _CI_RULES) if baseline
                           else _absent("baseline")),
        })
