...
from ..models import (
    C_AUTHN_AUTHZ, C_CREDENTIAL_STORAGE, C_INPUT_VALIDATION, C_TLS,
    Confidence, ScanSignalId, ScanSignalResult, ScanUpstream,
...
TLS_RULES: tuple[tuple[str, str, Confidence, re.Pattern[str], str], ...] = (
    ("ss1_tls_verification_disabled", "high", Confidence.HIGH, re.compile(
    ...
    ("ss1_weak_tls_version", "medium", Confidence.HIGH, re.compile(
    ...
    ("ss1_plaintext_http_url", "medium", Confidence.MEDIUM, re.compile(
...
def _observation(category: str, rule: str, severity: str,
                 confidence: Confidence, detail: str, path: str,
                 line: int | None, quote: str) -> SecurityObservation:
    return SecurityObservation(
        signal=ScanSignalId.SS1, category=category, rule=rule, detail=detail,
        ...
        for index, line in enumerate(lines, start=1):
            for rule, severity, confidence, pattern, detail in TLS_RULES:
                if pattern.search(line):
                    out.append(_observation(C_TLS, rule, severity, confidence,
                                            detail, path, index, line.strip()))
    return out
    ...
    for candidate in upstream.sources:
        ...
        for member in candidate.members:
            ...
            if member.path not in out or (
                    ...
                    and (current is None or member.line < current)):
                out[member.path] = member.line
    return out


def _validation(blobs: Mapping[str, str],
    ...
    for path, line in sorted(_entry_point_files(upstream).items()):
        ...
        quote = (lines[line - 1].strip() if line and line <= len(lines)
                 else path)
        out.append(SecurityObservation(
            signal=ScanSignalId.SS1, category=C_INPUT_VALIDATION,
            ...
            severity_hint="medium", path=path, line=line,
            evidence=quote[:_MAX_EVIDENCE], key="",
    ...
    return SignalOutput(
        row=ScanSignalResult(
            ...
            categories={
                ...
                C_INPUT_VALIDATION: (
                    Measurement.measured(float(len(validation))) if s3_ok
