...
from ..models import (
    C_DATA_SENSITIVITY, C_ENTITY_ACCESS, Confidence, EvidenceRef,
    ScanSignalId, ScanSignalResult, ScanUpstream, Sensitivity,
    SensitivityRecord, SignalOutput, SignalSource, family_of,
)
...
_FIELD_RULES: tuple[tuple[Sensitivity, str, frozenset[str]], ...] = (
    (Sensitivity.AUTHENTICATION, "ss4_authentication_field_name", frozenset({
        "password", "passwd", "passwordhash", "password_hash", "secret",
        "token", "access_token", "accesstoken", "refresh_token",
        "refreshtoken", "session", "session_id", "sessionid", "api_key",
        "apikey", "mfa", "totp", "otp", "salt", "credential", "credentials",
    })),
    (Sensitivity.FINANCIAL, "ss4_financial_field_name", frozenset({
        "card", "card_number", "cardnumber", "pan", "cvv", "cvc", "iban",
        "bic", "swift", "account_number", "accountnumber", "routing_number",
        "routingnumber", "balance", "amount", "currency", "invoice",
        "transaction", "payment_method", "paymentmethod", "card_last4",
        "cardlast4", "sort_code", "sortcode",
    })),
    (Sensitivity.HEALTH, "ss4_health_field_name", frozenset({
        "diagnosis", "medication", "prescription", "patient", "icd",
        "allergy", "blood_type", "bloodtype", "medical_record",
        "medicalrecord", "nhs_number", "nhsnumber",
    })),
    (Sensitivity.PII, "ss4_pii_field_name", frozenset({
        "email", "e_mail", "phone", "phone_number", "phonenumber", "mobile",
        "first_name", "firstname", "last_name", "lastname", "full_name",
        "fullname", "address", "street", "postcode", "zip", "zipcode",
        "ssn", "national_id", "nationalid", "passport", "date_of_birth",
        "dateofbirth", "dob", "ip_address", "ipaddress", "latitude",
        "longitude",
    })),
)

_REGULATORY = re.compile(
    r"\b(PCI[- ]?DSS|PCI\b|HIPAA|GDPR|SOC ?2|PSD2|CCPA|FERPA)\b")
...
def _accessors(entity: str, upstream: ScanUpstream) -> list[str]:
    ...
    return sorted({c.local_id for c in upstream.sources
                   if c.signal is ScanSignalId.S3
                   and normalize(c.name) == key})


def _gap(reason: str) -> SignalOutput:
    nc = Measurement.not_collected(reason)
    return SignalOutput(row=ScanSignalResult(
        signal=ScanSignalId.SS4, family=family_of(ScanSignalId.SS4),
...
def evaluate(blobs: Mapping[str, str],
    ...
    for decl in declarations(blobs):
        ...
        for field in decl.fields:
            ...
            for classification, rule, terms in _FIELD_RULES:
                if tokens & terms:
                    matched.setdefault(classification, (rule, []))[1].append(
                        field)
                    break
        accessed = _accessors(decl.name, upstream) if s3_ok else []
        for classification, (rule, fields) in matched.items():
            records.append(SensitivityRecord(
                classification=classification, entity=decl.name,
                origin=_origin(decl), fields=sorted(set(fields)),
                accessed_by=accessed,
                evidence=[EvidenceRef(path=decl.path, lines=str(decl.line))],
                rule=rule,
                confidence=(Confidence.HIGH if len(fields) > 1
                            else Confidence.MEDIUM)))
        if regulated:
            records.append(SensitivityRecord(
                classification=Sensitivity.REGULATORY, entity=decl.name,
                origin=_origin(decl), fields=sorted(decl.fields),
                accessed_by=accessed,
                evidence=[EvidenceRef(path=decl.path, lines=str(decl.line))],
    ...
    access = (Measurement.measured(
        float(sum(1 for r in records if r.accessed_by)))
        if s3_ok else upstream.gap(ScanSignalId.S3, "entity_access"))
    return SignalOutput(
        row=ScanSignalResult(
            signal=ScanSignalId.SS4, family=family_of(ScanSignalId.SS4),
            version=VERSION, source=SignalSource.COMPUTED,
            collected=classified,
            categories={C_DATA_SENSITIVITY: classified,
                        C_ENTITY_ACCESS: access}),
