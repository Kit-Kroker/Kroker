...
def build_parser() -> argparse.ArgumentParser:
    ...
    s.add_argument("--mode", choices=["greenfield", "brownfield"], default="brownfield")
    s.add_argument("--repo")
    s.add_argument(
        "--role-model",
        action="append",
    ...
    sa.add_argument(
        "--dry-run", action="store_true", help="print the plan without touching Temporal"
    )
    sa.add_argument(
    ...
    br.add_argument(
        ...
        default=None,
        dest="gate_policy",
        help="override the case's gate_policy for every gate in the child FeatureWorkflow",
    )
    ...
    bxn.add_argument(
        "--axis",
        required=True,
        choices=["prompt", "model", "harness", "schema", "tool_org", "memory"],
    )
...
async def main() -> None:
    ...
    if args.cmd == "eval":
        ...
        if args.view:
            ...
            if binary is None:
                raise SystemExit("promptfoo is not installed")
            subprocess.run([binary, "view"])
        return
