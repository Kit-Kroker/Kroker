...
def collect_node_ids(test_root: Path, prefix: str) -> list[str]:
    ...
    root = Path(test_root)
    out: list[str] = []
    discovered = sorted(set(root.rglob("test_*.py")) | set(root.rglob("*_test.py")))
...
def convert_repo(src: Path, dest_root: Path, *, judge_model: str) -> ImportReport:
    ...
    if dep_src is not None and not dep_src.is_file():
        raise FileNotFoundError(f"{repo_name}: declared dependencies path is not a file: {dep_src}")
    ...
    (case_dir / "oracle" / "conftest.py").write_text(ORACLE_CONFTEST, encoding="utf-8")
    ...
    declared_dirs = {cfg.unit_tests, cfg.acceptance_tests, cfg.usage_examples or "", "docs"}
    ...
    for entry in sorted(src.iterdir()):
        ...
        if entry.is_dir():
            ...
            reference_files += sum(1 for p in (ref / entry.name).rglob("*") if p.is_file())
        else:
