...
def _row_collected(scan: ScanResult, sid: ScanSignalId) -> Measurement:
    ...
    if row is None:
        return Measurement.not_collected(
            f"{sid.value} is not present in this scan")
...
def project(scan: ScanResult, tree_hash: str, commit_sha: str) -> CodebaseMap:
    ...
    if s5.state is CollectionState.MEASURED:
        modules = tuple(sorted(
            (MapModule(name=c.name,
                       member_paths=tuple(sorted({m.path for m in c.members
                                                  if m.path})),
        ...
        contracts = tuple(sorted(
            (MapContract(kind=m.kind, value=m.value, path=m.path, line=m.line)
             for c in scan.candidates for m in c.members
        ...
        modules, contracts = (), ()
        members = Measurement.not_collected(
            f"no modules: {_reason(s5, ScanSignalId.S5)}")
    ...
    if (qs2.state is CollectionState.MEASURED
            or qs3.state is CollectionState.MEASURED):
        spots: list[HotSpot] = []
        if qs3.state is CollectionState.MEASURED:
            spots.extend(
                HotSpot(path=f.path, source="testability",
                        reason=f"{f.severity}: {f.pattern}",
                        metric=Measurement.measured(
                            float(_SEVERITY_RANK[f.severity])))
                for f in scan.testability)
        if qs2.state is CollectionState.MEASURED:
            spots.extend(
                HotSpot(path=r.path, source="coverage",
                        reason=f"coverage from {r.source}"
                               f"{' (' + r.tool + ')' if r.tool else ''}",
                        metric=r.covered)
                for r in scan.coverage
                if r.scope == "file"
                and r.covered.state is CollectionState.MEASURED)
        hot_spots = tuple(sorted(
            spots, key=lambda h: (h.path, h.source, h.reason)))
        hot_collected = Measurement.measured(float(len(hot_spots)))
    else:
        hot_spots = ()
        hot_collected = Measurement.not_collected(
            f"no hot spots: {_reason(qs2, ScanSignalId.QS2)}; "
            f"{_reason(qs3, ScanSignalId.QS3)}")
    ...
    return CodebaseMap(
        ...
        modules_collected=members, contracts_collected=contracts_collected,
        hot_spots_collected=hot_collected, collected=collected)
